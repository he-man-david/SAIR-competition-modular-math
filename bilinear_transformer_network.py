...
class BilinearTransformerNetwork(nn.Module):
    ...
    def create_bilinear_context(
        ...
        if bilinear_key not in self.bilinear_w:
            raise ValueError(
                f"{bilinear_key} is not configured in "
                f"bilinear_w dict {self.bilinear_w}"
            )

        real_mask = ~self.get_pad_mask(
            key_value_tokens
        )
    ...
    def encode_inputs(
        ...
        memory_pad_mask = self.get_memory_pad_mask(
            a,
            b,
            p,
        )
    ...
    def forward(
        ...
        memory, memory_pad_mask = self.encode_inputs(
            a,
            b,
            p,
        )
